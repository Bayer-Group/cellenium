...
const OntologySandbox = () => {
  ...
  const [ontologyTrees, setOntologyTrees] =
    useState<Map<string, OntologyItem>>();
  ...
  return (
    <Stack>
      {ontologyData && (
        <OntologySelect
          handleChange={setSelectedOntology}
          ontologies={ontologyData.ontologiesList}
        />
      )}
      {ontologyTrees &&
        ontologyTrees.get(selectedOntology) !== undefined &&
        selectedOntology && (
          <OntologyTree
            ontology={ontologyTrees.get(selectedOntology) as any}
