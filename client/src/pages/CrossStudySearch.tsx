...
const CrossStudySearch = () => {
  ...
  const heatmapDisplayData = useMemo(() => {
    ...
    const studyLayerIdMap = new Map(
      studyList.map((s) => [s.defaultStudyLayerId, s]),
    );
  ...
  const onHeatmapClick = (
    dotPlotElement: DotPlotElementFragment,
      ...
      {omicsIds.length === 0 && (
        <Center>
          <Text style={{ width: "50em" }} color={"dimmed"}>
            Please enter your genes of interest. Cellenium will show the gene's
            expression in human studies with standardized cell annotation
            (CellO). As the study data is processed and normalized
            independently, this is a qualitative direction for which studies to
            explore independently. Click in the chart to open a study.
          </Text>
        </Center>
      )}
