...
function CoexpressionPlot({
  ...
  const setOmicsX = (omics: Omics | null) =>
    setOmicsAll((prev) =>
      prev.map((o, i) => (i === stateOffset * 2 ? omics : o)),
    );
  const setOmicsY = (omics: Omics | null) =>
    setOmicsAll((prev) =>
      prev.map((o, i) => (i === stateOffset * 2 + 1 ? omics : o)),
    );
  const { table, loading } = useExpressionValues(
    omicsX && omicsY ? [omicsX.omicsId, omicsY.omicsId] : [],
    false,
  );

  const celltypeDiscoveryCoexpressionSamples = useRecoilValue(
    celltypeDiscoveryCoexpressionSamplesState,
  );
  ...
  const preparedPlot = useMemo(() => {
    ...
    if (filterSampleIds) {
      ...
      sameSampleExprValues = sameSampleExprValues
        ...
        .filter((d: any, p: any) =>
          aq.op.includes(p.filterSampleIds, d.studySampleId, 0),
        );
    }
...
function CelltypeDiscovery() {
  ...
  const [omicsAll, setOmicsAll] = useRecoilState(celltypeDiscoveryGenesState);
  const [
    celltypeDiscoveryCoexpressionSamples,
    setCelltypeDiscoveryCoexpressionSamples,
  ] = useRecoilState(celltypeDiscoveryCoexpressionSamplesState);
  ...
  const [selectedSampleIds, setSelectedSampleIds] = useState<number[] | null>(
    null,
  );

  const onCoexpressionSelection = (
    event: Readonly<Plotly.PlotSelectionEvent>,
  ) => {
    const theelectedSampleIds = event.points.map(
      (p) => p.customdata,
    ) as number[];
  ...
  const newPlotBasedOnSelectedSamples = () => {
    ...
    setCelltypeDiscoveryCoexpressionSamples((prev) => [
      ...prev,
      selectedSampleIds,
    ]);
  ...
  const [saveUserAnnotationMutation, { loading: saveUserAnnotationLoading }] =
  ...
  const saveUserAnnotation = () => {
    if (study && selectedSampleIds && selectedSampleIds.length > 0) {
      const omicsX =
        omicsAll[(celltypeDiscoveryCoexpressionSamples.length - 1) * 2];
      const omicsY =
        omicsAll[(celltypeDiscoveryCoexpressionSamples.length - 1) * 2 + 1];
      saveUserAnnotationMutation({
        variables: {
          studyId: study.studyId,
          annotationGroupName: annotationName,
          selectedSampleIds: selectedSampleIds
            .filter((id) => id !== UNEXPRESSED_SAMPLE_ID)
            .join(","),
          unexpressedSamplesOmicsIds: (selectedSampleIds.indexOf(
            UNEXPRESSED_SAMPLE_ID,
          ) !== -1
            ? [omicsX?.omicsId, omicsY?.omicsId]
            : null) as InputMaybe<number[]>,
        },
      })
        .then(() => {
          showNotification({
            ...
            message:
              "Differentially expressed genes are calculated in the background and will appear later.",
            color: "green",
            autoClose: 2000,
          });
          setAnnotationName("");
    ...
    <Group position={"apart"}>
      <LeftSidePanel>
        <Stack>
          ...
          {annotationGroupId && (
            <AnnotationGroupDisplay disableSelection={true} />
          )}
        </Stack>
      </LeftSidePanel>
      <main>
        <ProjectionPlot
          colorBy={"annotation"}
          showSampleIds={selectedSampleIds}
          disableSelection={true}
        />
      </main>
      <RightSidePanel>
        <Stack align={"flex-start"} justify={"flex-start"} spacing={"md"}>
          {[...Array(celltypeDiscoveryCoexpressionSamples.length)].map(
            (_, i) => (
          ...
          <Button
            ...
            disabled={
              selectedSampleIds === null || selectedSampleIds.length === 0
            }
          >
            Add plot, based on selected samples
          </Button>
          <Button
            onClick={removeLastPlot}
            disabled={celltypeDiscoveryCoexpressionSamples.length < 2}
          >
            Remove last plot
          </Button>
          <Group>
