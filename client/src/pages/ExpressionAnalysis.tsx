...
function ProjectionPlots() {
  ...
  const tablePerGene = useMemo(() => {
    ...
    return [...selectedGenes]
      ...
      .map((g) =>
        table
          .params({ omicsId: g.omicsId })
          .filter((d: any, p: any) => d.omicsId === p.omicsId),
      );
  }, [selectedGenes, table]);
      ...
      {tablePerGene &&
        [...selectedGenes].reverse().map((g, i) => (
          <Stack key={g.omicsId} align={"center"}>
            ...
            <ProjectionPlot
              colorBy={"expression"}
              expressionTable={tablePerGene[i]}
            />
          </Stack>
        ))}
    </Group>
...
function DotPlots() {
  ...
  const heatmapDisplayData = useMemo(() => {
    ...
    return data.expressionByAnnotationList.map((dotPlotElement) => ({
      ...dotPlotElement,
      displaySymbol:
        study?.studyOmicsMap?.get(dotPlotElement.omicsId)?.displaySymbol ||
        "nn",
    }));
  }, [data, selectedGenes]);
      ...
      {heatmapDisplayData && (
        <Center style={{ height: "100%", width: "100%" }}>
          <ExpressionDotPlot
            ...
            annotationTitle={
              study?.annotationGroupMap.get(annotationGroupId || -1)
                ?.displayGroup || "group"
            }
            xAxis={"displaySymbol"}
          />
...
const ExpressionAnalysis = () => {
  ...
  return (
    <Group align={"flex-start"} position={"apart"} spacing={"xs"} noWrap>
      <LeftSidePanel>
        <Stack spacing={"md"}>
          <ExpressionAnalysisTypeSelectBox
            handleSelection={setAnalysisType}
