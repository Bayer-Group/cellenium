...
const AnnotationComparison = () => {
  ...
  return (
    <Group position={"apart"} noWrap>
      <LeftSidePanel>
        <SankeyAnnotationGroupSelector
          annotationGroups={annotations}
          handleChange1={handleChange1}
          value1={value1}
          handleChange2={setValue2}
          value2={value2}
        />
      </LeftSidePanel>
      <main
      ...
      >
        {study && annotations.length >= 2 && value1 && value2 && (
          <SankeyPlot
            annotationValues1={
              study.annotationGroupMap.get(parseInt(value1))
                ?.annotationValuesList
            }
            annotationValues2={
              study.annotationGroupMap.get(parseInt(value2))
                ?.annotationValuesList
            }
        ...
        {annotations.length < 2 && (
          <Center style={{ height: "100%", width: "100%" }}>
            <Text>
              My friend. Please think twice: Does a comparison of annotation
              groups make sense when there is only 1???
            </Text>
          </Center>
        )}
      </main>
