...
export function StudyAdmin() {
  ...
  const [selectedEditStudy, setSelectedEditStudy] = useState<
    StudyAdminDetailsFragment | undefined
  >(undefined);
  const [selectedDeleteStudy, setSelectedDeleteStudy] = useState<
    StudyAdminDetailsFragment | undefined
  >(undefined);
  const [selectedLogStudy, setSelectedLogStudy] = useState<
    StudyAdminDetailsFragment | undefined
  >(undefined);
    ...
    },
    {
      ...
      cell: (row: StudyAdminDetailsFragment) =>
        !row.hasImportLog ? null : (
          <IconArticle
            onClick={() => setSelectedLogStudy(row)}
    ...
    },
    {
      ...
      cell: (row: StudyAdminDetailsFragment) =>
        !row.adminPermissionGranted ? null : (
          <IconDotsVertical
            onClick={() => setSelectedEditStudy(row)}
            style={{ cursor: "pointer" }}
          />
        ),
    },
  ...
  return (
    <Container fluid={true}>
      <EditStudyModal
        opened={selectedEditStudy !== undefined}
        reset={resetEditModal}
        study={selectedEditStudy}
      />
      ...
      />
      <StudyLogModal
        opened={selectedLogStudy !== undefined}
        study={selectedLogStudy}
        reset={() => setSelectedLogStudy(undefined)}
      />
      <NavBar />
      <Space h="xl" />
      <Stack px="md">
