...
function SearchBar({ ontologies, onSearchElementsUpdate }: Props) {
  ...
  const [getAutocomplete, { data: autocompleteSuggestions, loading }] =
    useAutocompleteLazyQuery();

  useEffect(() => {
    if (autocompleteSuggestions) {
      let newOfferings: OfferingItem[] =
  ...
  useEffect(
    () => onSearchElementsUpdate(selectedFilters.map((f) => f.ontcode)),
    [selectedFilters],
  );
  ...
  function handleSubmit(item: OfferingItem) {
    ...
    let check = selectedFilters.filter(
      (e) => e.ontology === item.ontology && e.ontcode === item.ontcode,
    );
    if (check.length === 0) {
  ...
  function handleFilterRemove(filter: OfferingItem) {
    let newFilters = selectedFilters.filter(
      (f) => !(f.ontcode === filter.ontcode && f.ontology === f.ontology),
    );
    ...
    <Group position={"left"} align={"flex-end"} spacing={4}>
      ...
      <Stack spacing={0} style={{ flexGrow: 1 }}>
        <Text size={"xs"} weight={800}>
          Filter studies by disease (MESH), tissue (NCIT), species, cell type
          (CO)
        </Text>
        ...
        >
          {loading ? (
            <Loader size={25} color={theme.colors.gray[5]} />
          ) : (
          ...
          <Group spacing={2}>
            {selectedFilters.map((filter) => {
              return (
                <SearchBadge
                  key={`${filter.ontology}_${filter.ontcode}`}
