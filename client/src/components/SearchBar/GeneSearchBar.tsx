...
const sortAlphaNum = (a: Omics, b: Omics) =>
  a.displaySymbol.localeCompare(b.displaySymbol, "en", { numeric: true });
...
function GeneSearchBar({ humanOnly, onGeneSelection }: Props) {
  ...
  const speciesList = useMemo(
    () => (humanOnly ? SPECIES.filter((s) => s.value === "9606") : SPECIES),
    [],
  );
  ...
  function handleChange(input: string) {
    ...
    const newOfferings = _.uniqBy(
      Array.from(allGenes.values())
        ...
        .filter((gene) =>
          gene.displaySymbol.toLowerCase().startsWith(input.toLowerCase()),
        )
  ...
  return (
    <Group position={"left"} align={"flex-end"} spacing={4} noWrap>
      ...
      <Stack spacing={0} style={{ flexGrow: 1 }}>
        ...
        >
          {!allGenes ? (
            <Loader variant={"dots"} size={25} color={theme.colors.gray[5]} />
          ) : (
