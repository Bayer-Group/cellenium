...
const ProjectionPlot = ({
  ...
  const [highlightAnnotation, setHighlightAnnotation] = useRecoilState(
    highlightAnnotationState,
  );
  ...
  const annotationProjectionData = React.useMemo(() => {
    ...
    samplesAnnotationProjectionTable = samplesAnnotationProjectionTable
      .join_right(
        study.samplesProjectionTables.get(projection),
        "studySampleId",
      )
    ...
    const distinctAnnotationValueIds: number[] =
  ...
  const annotationTraces = React.useMemo(() => {
    ...
    return annotationProjectionData.distinctAnnotationValueIds.map(
      (annotationValueId) => {
        const tableForAnnotation =
          annotationProjectionData.samplesAnnotationProjectionTable
            .params({ annotationValueId })
            .filter(
              (d: any, p: any) => d.annotationValueId === p.annotationValueId,
            );
        return {
          type: "scattergl",
          ...
          customdata:
            study.annotationValueMap.get(annotationValueId) &&
            tableForAnnotation.array("annotationValueId", Int32Array),
          text: study.annotationValueMap.get(annotationValueId)?.displayValue,
          mode: "markers",
          marker: {
            size: 3,
            opacity:
              expressionTable || (showSampleIds && showSampleIds.length > 0)
                ? 0.2
                : 0.7,
            color:
              study.annotationValueMap.get(annotationValueId)?.color ||
              "#d7d5d5",
          },
          showlegend: false,
          hoverinfo: "text",
  ...
  const annotationHighlightTrace = React.useMemo(() => {
    ...
    const tableForAnnotation =
      annotationProjectionData.samplesAnnotationProjectionTable
        ...
        .filter(
          (d: any, p: any) => d.annotationValueId === p.highlightAnnotation,
        );
    return {
      type: "scattergl",
      x: tableForAnnotation.array("projectionX", Float32Array),
  ...
  const selectedAnnotationHighlightTrace = React.useMemo(() => {
    ...
    const tableForAnnotation =
      annotationProjectionData.samplesAnnotationProjectionTable
        ...
        .filter(
          (d: any, p: any) => d.annotationValueId === p.selectedAnnotation,
        );
    return {
      type: "scattergl",
      x: tableForAnnotation.array("projectionX", Float32Array),
      y: tableForAnnotation.array("projectionY", Float32Array),
      mode: "markers",
      text: study.annotationValueMap.get(selectedAnnotation as number)
        ?.displayValue,
      marker: {
        size: 10,
        opacity: 0.1,
  ...
  const selectedSamplesTrace = React.useMemo(() => {
    ...
    const filteredTable =
      annotationProjectionData.samplesAnnotationProjectionTable
        ...
        .filter((d: any, p: any) =>
          aq.op.includes(p.showSampleIds, d.studySampleId, 0),
        )
        .reify();
    return {
      type: "scattergl",
