...
const columns = memoize((clickHandler, handleColorClick) => [
  {
    name: "identifier",
    selector: (row: any) => (
      <Text title={row.displaySymbol}>{row.displaySymbol}</Text>
    ),
    sortable: true,
    width: "80px",
  },
      ...
      return (
        <Group key={id} spacing={"xs"}>
          ...
          <ActionIcon
            title={"add to gene store"}
            color={"blue.3"}
            onClick={() => addToStore(gene as Omics)}
            size="xs"
            variant={"default"}
          >
            <IconPlus size={12} color={"black"} />
...
const DEGTable = ({ annotationId }: Props) => {
  ...
  return (
    <Stack justify={"flex-start"} align={"center"} w={"100%"}>
      ...
      {data &&
        study &&
        data.differentialExpressionVsList.length > 0 &&
        study.omicsTypes.length > 1 && (
          <DataTable
            dense
